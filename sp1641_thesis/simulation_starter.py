...
#DATA COLLECTION, FUNCTIONS THAT ARE RELATED IN COLLECTING DATA, AND VISUALISING ARE HERE
    ...
    def collect_travel_time_to_number_of_bots_in_shortest_paths(self)->None:
        ...
        num_of_bots_in_shortest_path = sum([1 for bot in temp_bots_array if bot.bot_info["Path Taken"] in self.shortest_path])
        num_of_bots_in_other_paths = abs(self.num_of_bots - num_of_bots_in_shortest_path) #abs is make sure that it always stays positive
        ...
        self.ratio_of_bots_on_shortest_path_to_other_paths.append(bot_ratio_of_other_paths_to_num_of_of_bots_shortest_paths)
    ...
    def shortening_path_list(self)->None:
        ...
        while len(self.all_paths_list)>self.max_amount_of_paths_all_paths_list:
            ...
            if more_var == fastest_time:
                print("weird issue has been achieved where the percent more value is same as fastest time yet the amount of paths has not been reduced")
                breakpoint()
